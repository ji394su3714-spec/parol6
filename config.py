...
# 取代原本平行的 STL_FILES 與 MESH_JOINT_INDICES
ROBOT_LINKS_CONFIG = {
    0: ['base_link.STL'],
    1: ['Link1_main.STL', 'Link1_motor.STL', 'Link1_screws.STL', 'Link1_gear.STL', 'Link1_cage.STL'],
    2: ['Link2_main.STL', 'Link2_motor.STL', 'Link2_screws.STL', 'Link2_gear.STL', 'Link2_cage.STL'],
    3: ['Link3_main.STL', 'Link3_motor.STL', 'Link3_screws.STL'],
    4: ['Link4_main.STL', 'Link4_motor.STL', 'Link4_screws.STL'],
    5: ['Link5_main.STL', 'Link5_motor.STL'],
    6: ['Link6_main.STL']
}
...
class Robot3DView(QWidget):
    ...
    def _assemble_statically(self):
        ...
        self.actor_joint_indices = []  # 新增：動態紀錄每個視覺物件對應的關節索引

        # 改用兩層迴圈解析字典
        for joint_idx, filenames in ROBOT_LINKS_CONFIG.items():
            for filename in filenames:
                ...
                # 直接使用字典的 Key (joint_idx) 來取得初始矩陣
                M0 = zero_matrices[joint_idx] if joint_idx < len(zero_matrices) else np.eye(4)
                ...
                self.actor_joint_indices.append(joint_idx)  # 自動同步紀錄索引
    ...
    def update_joints(self, joint_angles, tcp_mat=None):
        ...
        # 將原本寫死的 MESH_JOINT_INDICES 改為 zip 遍歷自動生成的 actor_joint_indices
        for actor, mat_idx in zip(self.actors, getattr(self, 'actor_joint_indices', [])):
