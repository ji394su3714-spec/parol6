...
class ParolCamWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Parol CAM - 六軸雷射切管核心 (專業 Splitter 架構)")
    ...
    def _setup_ui(self):
        ...
        self.btn_tb_magic = QPushButton("萃取")
        self.btn_tb_magic.setCheckable(True) # 讓按鈕可以像開關一樣被「按下去」
        self.btn_tb_magic.toggled.connect(self.on_magic_wand_toggled)
        self.btn_tb_move = QPushButton("移動")
        self.btn_tb_scale = QPushButton("縮放")
        self.btn_tb_mirror = QPushButton("鏡射")
        self.btn_tb_add = QPushButton("增加實體")
        
        # 按鈕背景設為更深的透明黑，移上去變色
        tb_style = """
            QPushButton { background-color: rgba(60, 60, 60, 255); color: white; padding: 8px 15px; border-radius: 4px; font-weight: bold; border: none; } 
            QPushButton:hover { background-color: rgba(90, 90, 90, 255); }
        ...
        # 將懸浮工具列也放進第 0 行第 0 列！並設定對齊「上方與置中」
        right_layout.addWidget(self.prepare_toolbar, 0, 0, Qt.AlignTop | Qt.AlignHCenter)
    ...
    def _setup_tab_prepare(self):
        """填充「2. 準備」面板內容"""
        prepare_layout = QVBoxLayout(self.page_prepare) # 直接把佈局塞進左側面板
        prepare_layout.setContentsMargins(10, 10, 10, 10)
        
        # 0. 系統與刀具綁定
        sys_group = QGroupBox("0. 系統與刀具 (TCP) 綁定")
        ...
        # 1. 匯入模型位置與姿態校正
        model_group = QGroupBox("1. 模型位置與姿態校正")
        ...
        # 2. 表面加工姿態微調
        offset_group = QGroupBox("2. 表面加工姿態微調")
        ...
        for i in range(3):
            ...
            offset_layout.addLayout(layout_o)
        offset_group.setLayout(offset_layout)

        # 3. STL 載入與萃取
        stl_group = QGroupBox("3. 幾何處理")
        stl_layout = QVBoxLayout()
        self.btn_load_stl = QPushButton("載入 STL 檔案")
        self.btn_load_stl.setStyleSheet("background-color: #5c3a21; color: white; padding: 8px;")
        self.btn_load_stl.clicked.connect(self.on_load_stl_clicked) 
        
        self.btn_generate_path = QPushButton("A. 幾何萃取 (生成切削路徑)")
        self.btn_generate_path.setStyleSheet("font-weight: bold; padding: 8px;")
        self.btn_generate_path.clicked.connect(self.on_generate_path_clicked)
        
        stl_layout.addWidget(self.btn_load_stl)
        stl_layout.addWidget(self.btn_generate_path)
        ...
        prepare_layout.addWidget(stl_group)
        prepare_layout.addStretch()

    def _setup_tab_preview(self):
        """填充「預覽」面板內容"""
        preview_layout = QVBoxLayout(self.page_preview) # 佈局塞進左側預覽面板
        preview_layout.setContentsMargins(10, 10, 10, 10)

        bake_group = QGroupBox("1. 軌跡運算")
        bake_layout = QVBoxLayout()
        self.btn_bake_ik = QPushButton("計算 IK 並烘焙腳本")
        self.btn_bake_ik.setStyleSheet("background-color: #b37700; color: white; padding: 15px; font-weight: bold;")
        ...
        player_group = QGroupBox("2. 軌跡動畫預覽")
    ...
    def on_slider_moved(self, value):
        if not self.baked_trajectory: return
        total_frames = len(self.baked_trajectory) - 1
        self.lbl_frame.setText(f"{value} / {total_frames}")
        target_joints = self.baked_trajectory[value]
        if self.robot_view:
            self.robot_view.update_joints(target_joints, tcp_mat=self.tcp_manager.get_active_matrix())
    ...
    def on_bake_ik_clicked(self):
        ...
        try:
            ...
            # ==========================================
            # 👑 智慧防彈解析層：精準挖出真正的 Waypoints
            # ==========================================
            waypoints = []
            if isinstance(script_data, list):
                # 檢查第一筆資料，如果它有 'type'，代表這是「指令區塊陣列」
    ...
    def on_generate_path_clicked(self):
        rx, ry, rz = [spin.value() for spin in self.offset_spins]
        try:
            locations, matrices = self.cam_engine.extract_contour_path(offset_rx=rx, offset_ry=ry, offset_rz=rz)
    ...
    def on_mesh_picked(self, hit_face_idx):
        """當滑鼠點擊確認時"""
        self.btn_tb_magic.setChecked(False)
        self.hover_visual.visible = False # 點擊後把高亮隱藏
    ...
    def on_magic_wand_toggled(self, checked):
     ...
     if checked:
         self.btn_tb_magic.setStyleSheet("background-color: #b37700; color: white; padding: 6px 15px; border-radius: 4px; font-weight: bold; border: none;")
         self.mesh_picker.toggle_picking_mode(True)
         self.robot_view.canvas.native.setCursor(QCursor(Qt.CrossCursor))
     else:
         self.btn_tb_magic.setStyleSheet("""
             QPushButton { background-color: rgba(60, 60, 60, 255); color: white; padding: 8px 15px; border-radius: 4px; font-weight: bold; border: none; } 
             QPushButton:hover { background-color: rgba(90, 90, 90, 255); }
             QPushButton:pressed { background-color: #b37700; }
         """)
