...
// 1. 關節配置與硬體參數
// 針對 TMC2209 / TMC5160 增加：獨立電流設定、待機降流比例
struct JointConfig {
...
// 綜合配置表 (整合極限開關、歸零參數與電流設定)
const JointConfig JOINTS[6] = {
    {54, 55, 38,  2,  LOW,  250, -120, 15000, 400, 1000, 0.25f}, // J1 (2209)
    {60, 61, 56, 12,  HIGH, -500,  50, 19000, 400,  950,  0.5f}, // J2 (5160)
    {43, 48, 58, 14,  HIGH,  500, -70, 17000, 400,  850,  0.5f}, // J3 (2209)
    {26, 28, 24, 15,  LOW,    0,   0, 14000, 300,  850, 0.25f}, // J4 (2209)
    {36, 34, 30, 63,  HIGH,    0,   0, 14000, 300,  850, 0.25f}, // J5 (2209)
    {59, 57, 40, 64,  LOW,    0,   0, 20000, 300,  680, 0.25f}  // J6 (2209)
};
...
TMC2209Stepper driver_J6(&serial_J6, R_SENSE_2209, 0);
        ...
        if (homingState[i] == 1 && JOINTS[i].limitPin != 0) {
            if (digitalRead(JOINTS[i].limitPin) == JOINTS[i].limitActiveState) {
                ...
                if (digitalRead(JOINTS[i].limitPin) == JOINTS[i].limitActiveState) {
                    ...
                    steppers[i]->stop(); // 【重要修正】取代 doSteps(0)
                    homingState[i] = 2; 
                    Serial.print(">>> J"); Serial.print(i + 1); Serial.println(" Hit! Waiting...");
                }
            } 
        }
        // 狀態 2：急停完畢，互相等待，確認大家都煞車後才退回 Offset
...
// 初始化 Setup
void setup() {
    Serial.begin(250000); // 配合上位機的高速鮑率
    SPI.begin();
    ...
    setupTMC2209(driver_J6, JOINTS[5].runCurrent_mA, JOINTS[5].holdCurrentRatio);
...
void processCommand() {
    ...
    for (int i = 0; i < 6; i++) {
        if (receivedAngles[i] == 999.0) {
            if (JOINTS[i].limitPin != 0 && JOINTS[i].homingSpeed != 0) {
                if (homingState[i] == 0) {
                    homingState[i] = 1; 
                    steppers[i]->setRampLen(0); 
